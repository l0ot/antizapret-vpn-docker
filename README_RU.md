...
3) Adguardhome перенаправляет запросы для заблокированных доменов в Go-бинарник dnsmap.
4) Go-бинарник dnsmap:
...
4. CoreDNS отправляет DNS-запрос на внутренний сервер dnsmap (контейнер antizapret), а dnsmap отправляет запрос обратно в adguard.
...
6. Если домен в белом списке - adguard разрешит его адрес и вернет в dnsmap.
7. Если домен не в белом списке, adguard возвращает SERVFAIL.
8. dnsmap отправляет ответ в adguard:
...
## Добавление ASN
Добавьте номера ASN (`AS13335` или `13335`) либо часть названия организации (`Cloudflare`) в
`./config/antizapret/custom/include-asn-custom.txt`. Для зарубежного узла используйте
`include-asn-world-custom.txt`. IP-адреса ответов DNS, принадлежащие указанным ASN, будут
направлены через соответствующий VPN-узел. Названия организаций сопоставляются с исходным
названием из MaxMind как регистронезависимая подстрока. Используйте формат `/regex pattern/`,
чтобы сопоставлять регистронезависимым регулярным выражением, например `/\bg-?core\b/`.
